...
If you discover a security vulnerability in TARS, please report it by:
...
chmod 600 ~/.TARS/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.TARS/config.json` with file permissions set to `0600`
...
- ✅ Run TARS with a dedicated user account
...
- Keep authentication data in `~/.TARS/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for TARS and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash TARS
   sudo -u TARS TARS gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.TARS
   chmod 600 ~/.TARS/config.json
   chmod 700 ~/.TARS/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.TARS/logs/TARS.log
   ```
...
- **Chat history is stored locally** - protect the `~/.TARS` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.TARS/logs/TARS.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/TARS/security/advisories
- Release Notes: https://github.com/HKUDS/TARS/releases

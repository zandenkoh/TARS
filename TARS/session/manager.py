...
from TARS.config.paths import get_legacy_sessions_dir
from TARS.utils.helpers import ensure_dir, safe_filename
...
class SessionManager:
    ...
    def _get_legacy_session_path(self, key: str) -> Path:
        """Legacy global session path (~/.TARS/sessions/)."""
        safe_key = safe_filename(key.replace(":", "_"))
    ...
    def delete_session(self, key: str) -> None:
        ...
        if legacy_path.exists():
            try:
                ...
                logger.warning(f"Failed to delete legacy session file {legacy_path}: {e}")
    ...
    def search_sessions(self, query: str) -> list[dict[str, Any]]:
        ...
        # ⚡ Bolt Optimization: Pre-compiled case-insensitive regex pattern avoids repeated
        # compilation and expensive per-line .lower() calls when scanning long files
        search_pattern = re.compile(re.escape(query), re.IGNORECASE).search

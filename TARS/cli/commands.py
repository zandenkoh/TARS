...
from TARS import __logo__, __version__
from TARS.cli.stream import StreamRenderer, ThinkingSpinner
from TARS.config.paths import get_workspace_path, is_default_workspace
from TARS.config.schema import Config
from TARS.utils.helpers import sync_workspace_templates

app = typer.Typer(
    name="TARS",
    context_settings={"help_option_names": ["-h", "--help"]},
    help=f"{__logo__} TARS - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from TARS.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} TARS[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} TARS[/cyan]"),
...
def onboard(
    ...
    """Initialize TARS configuration and workspace."""
    from TARS.config.loader import get_config_path, load_config, save_config, set_config_path
    from TARS.config.schema import Config
    ...
    if wizard:
        from TARS.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'TARS onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'TARS agent -m "Hello!"'
    gateway_cmd = "TARS gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} TARS is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _instantiate_provider(config: Config, p: Any, name: str, model: str):
    ...
    if backend == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            console.print(f"[red]Error: Azure OpenAI ({name}) requires api_key and api_base.[/red]")
            raise typer.Exit(1)
    elif backend == "openai_compat" and not model.startswith("bedrock/"):
        needs_key = not (p and p.api_key)
        exempt = spec and (spec.is_oauth or spec.is_local or spec.is_direct)
        if needs_key and not exempt:
            console.print(f"[red]Error: No API key configured for provider '{name}'.[/red]")
            raise typer.Exit(1)
...
def _make_provider(config: Config):
    ...
    Supports failover if `priority` is configured in `agents`.
    """
    ...
    if not failover_configs:
        ...
        if not provider_name or not p:
            console.print("[red]Error: No valid provider found for model.[/red]")
            raise typer.Exit(1)
        failover_configs = [(p, provider_name, model)]

    if len(failover_configs) == 1:
        p, name, model, tokens = failover_configs[0]
        ...
        for p, name, model, tokens in failover_configs:
            ...
            # User request: "Make sure max 1 retry per provider."
            # Our default is (1, 2, 4) i.e. 3 retries.
            # (2,) means 1 retry (after 2s delay).
            instance._CHAT_RETRY_DELAYS = (2,)
            providers.append((instance, model, tokens))
        provider = FailoverProvider(providers)
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from TARS.config.loader import load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from TARS.config.paths import get_cron_dir
...
def gateway(
    ...
    from TARS.cron.types import CronJob
    ...
    console.print(f"{__logo__} Starting TARS gateway version {__version__} on port {port}...")
    sync_workspace_templates(config.workspace_path)
    bus = MessageBus(config=config)
    ...
    daily_time = config.skills.tasks.daily_time
    try:
        ...
        if not any(j.name == "Daily Task Generation" for j in existing_jobs):
            ...
            cron.add_job(
                ...
                message="Generate my daily task list for today. Summary first, then JSON task data.",
                deliver=True,
            )
    except Exception as e:
        logger.error("Failed to register daily task job: {}", e)

    # Create channel manager
    channels = ChannelManager(config, bus)

    def _pick_default_target() -> tuple[str, str]:
        """Pick a routable channel/chat target for scheduled/proactive messages."""
...
def _get_bridge_dir() -> Path:
    ...
    from TARS.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # TARS/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall TARS")
        raise typer.Exit(1)
...
def status():
    """Show TARS status."""
    from TARS.config.loader import get_config_path, load_config

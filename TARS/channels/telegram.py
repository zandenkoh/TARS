...
from telegram import BotCommand, CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup, ReactionTypeEmoji, ReplyParameters, Update
...
class TelegramConfig(Base):
    ...
    streaming: bool = True
    tts: bool = True
    tts_engine: Literal["local", "gemini"] = "local"
...
class TelegramChannel(BaseChannel):
    ...
    BOT_COMMANDS = [
        ...
        BotCommand("status", "Show uptime, model, MCPs"),
        BotCommand("tools", "List tools and MCP servers"),
        BotCommand("zapier", "List available Zapier actions"),
        BotCommand("calendar", "Use Zapier to create an event"),
        BotCommand("email", "Use Zapier Gmail"),
        BotCommand("note", "Save a note using filesystem"),
        BotCommand("journal", "Save a journal using filesystem"),
        BotCommand("summary", "Summarize files or recent chat"),
        BotCommand("schedule", "Manage scheduled tasks"),
        BotCommand("search", "Search using DuckDuckGo"),
        BotCommand("browser", "Use Browser MCP"),
        BotCommand("tasks", "Manage daily tasks"),
    ]
    ...
    async def start(self) -> None:
        ...
        self._app.add_handler(CommandHandler("tools", self._forward_command))
        self._app.add_handler(CommandHandler("zapier", self._forward_command))
        self._app.add_handler(CommandHandler("calendar", self._forward_command))
        self._app.add_handler(CommandHandler("email", self._forward_command))
        self._app.add_handler(CommandHandler("note", self._forward_command))
        self._app.add_handler(CommandHandler("journal", self._forward_command))
        self._app.add_handler(CommandHandler("summary", self._forward_command))
        self._app.add_handler(CommandHandler("schedule", self._forward_command))
        self._app.add_handler(CommandHandler("search", self._forward_command))
        self._app.add_handler(CommandHandler("browser", self._forward_command))
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if isinstance(reply_markup, (dict, list)):
            try:
                ...
                logger.warning("Failed to parse reply_markup dict: {}", e)
                reply_markup = None
    ...
    async def _send_tts(self, chat_id: int, text: str, reply_params=None, thread_kwargs=None) -> None:
        ...
        try:
            ...
            logger.info("Successfully sent local TTS voice message!")
    ...
    async def _send_gemini_tts(self, chat_id: int, text: str, reply_params=None, thread_kwargs=None) -> None:
        ...
        try:
            ...
            # Lookup Gemini API key from global config
            bus_config: Config = getattr(self.bus, "config", None)
            api_key = None
            if bus_config:
            ...
            os.unlink(audio_path)
            logger.info("Successfully sent Gemini Native voice message!")
        except Exception as e:
        ...
        if meta.get("_stream_end"):
            ...
            try:
                ...
                try:
                    ...
                    pass
            # --- Auto-TTS voice message generation for streamed completion ---
            await self._send_tts(int_chat_id, buf.text)
            return
    ...
    async def _on_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            ...
            "/status — Show uptime, current model, connected MCPs (filesystem + Zapier)\n"
            "/tools — List all tools and MCP servers with short description\n"
            "/zapier — List available Zapier actions\n"
            "/calendar — Use Zapier to create event — ALWAYS ask for confirmation first\n"
            "/email — Use Zapier Gmail — ALWAYS ask for confirmation\n"
            "/note or /journal — Use filesystem MCP in ~/TARS-safe-files\n"
            "/summary — Summarize files or recent chat\n"
            "/schedule — Use cron/heartbeat\n"
            "/search or /browser — Use DuckDuckGo or Browser MCP\n"
    ...
    async def _on_tasks(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if tasks_data:
            ...
            await self._send_text(message.chat_id, text, reply_markup=markup)
        else:
            # No tasks found, prompt the agent to generate them
            await self._handle_message(
                sender_id=self._sender_id(user),
                chat_id=str_chat_id,
                content="Generate my daily task list for today and show it to me.",
                metadata=self._build_message_metadata(message, user),
                session_key=self._derive_topic_session_key(message),
            )
    ...
    async def _on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if data_parts[0] == "task":
            ...
            if action in ("done", "skip"):
                ...
                await self._handle_message(
                    ...
                    content="I want to add a task to my daily list.",
                ...
                date_str = data_parts[2]
                text = "📅 *Select Daily Task Generation Time*\n\nChoose when you want the AI to generate your daily task list each morning:"
                markup = manager.format_time_selection_markup(date_str)
                try:
                    html = _markdown_to_telegram_html(text)
                    await query.edit_message_text(text=html, parse_mode="HTML", reply_markup=markup)
                except Exception:
        ...
        # 2. Handle generic callback queries by forwarding to the AI agent
        user = query.from_user

...
from TARS.security.network import validate_url_target, validate_resolved_url
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — TARS uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use TARS get_media_dir("qq").
    media_dir: str = ""
...
class QQChannel(BaseChannel):
    ...
    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        if not self._http:
            self._http = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120))
        try:
            target_url = media_ref
            resp = None
            for _ in range(5):
                resp = await self._http.get(target_url, allow_redirects=False)
                if resp.status in (301, 302, 303, 307, 308):
                    loc = resp.headers.get("Location")
                    if not loc:
                        break
                    target_url = urljoin(target_url, loc)
                    ok, err = validate_resolved_url(target_url)
                    if not ok:
                        logger.warning("QQ outbound media redirect SSRF blocked: {} -> {}", media_ref, target_url)
                        await resp.release()
                        return None, None
                    await resp.release()
                    continue
            ...
            if not resp or resp.status != 200:
                if resp:
                    logger.warning("QQ outbound media download failed status={} url={}", resp.status, target_url)
                    await resp.release()
                return None, None
            ...
            if not data:
                return None, None
            filename = os.path.basename(urlparse(target_url).path) or "file.bin"
        ...
        try:
            ...
            for _ in range(5):
                ...
                if resp.status in (301, 302, 303, 307, 308):
                    ...
                    ok, err = validate_resolved_url(target_url)
                    if not ok:
                        logger.warning("QQ download redirect SSRF blocked: {} -> {}", url, target_url)
                        await resp.release()
                        return None
                    await resp.release()
                    continue
            ...
            if not resp or resp.status != 200:
                if resp:
                    logger.warning("QQ download failed: status={} url={}", resp.status, target_url)
                    await resp.release()
                return None

        ...
        try:
            ...
            while redirect_count < 5:
                ...
                if resp.status in (301, 302, 303, 307, 308):
                    ...
                    if not ok:
                        logger.warning("QQ outbound media SSRF blocked on redirect url={} err={}", next_url, err)
                        return None, None
            ...
            logger.warning("QQ outbound media download failed: too many redirects url={}", media_ref)
            return None, None
        except Exception as e:
        ...
        try:
            ...
            while redirect_count < 5:
                ...
                if resp.status in (301, 302, 303, 307, 308):
                    ...
                    ok, err = validate_resolved_url(next_url)
                    if not ok:
                        logger.warning("QQ download SSRF blocked on redirect url={} err={}", next_url, err)
                        return None
                    current_url = next_url
                    redirect_count += 1
                elif resp.status != 200:
            ...
            if resp is None or resp.status != 200:
                if resp:
                    resp.close()
                logger.warning("QQ download failed: too many redirects or error url={}", url)
                return None

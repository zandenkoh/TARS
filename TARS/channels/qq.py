...
class QQChannel(BaseChannel):
    ...
    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        try:
            url = media_ref
            for _ in range(5):
                resp = await self._http.get(url, allow_redirects=False)
                if resp.status in (301, 302, 303, 307, 308):
                    ...
                    if not ok:
                        logger.warning("QQ outbound media redirect SSRF blocked url={} err={}", url, err)
                        resp.release()
                        return None, None
                    resp.release()
                    continue
        ...
        try:
            ...
            for _ in range(5):
                ...
                if resp.status in (301, 302, 303, 307, 308):
                    ...
                    if not ok:
                        logger.warning("QQ download redirect SSRF blocked url={} err={}", target_url, err)
                        resp.release()
                        return None
                    resp.release()
                    continue

...
class QQChannel(BaseChannel):
    ...
    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        if not self._http:
            self._http = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120))
        try:
            # Manually handle redirects to prevent SSRF via redirect target
            current_url = media_ref
            resp = None
            try:
                for _ in range(5):
                    ok, err = validate_url_target(current_url) if current_url == media_ref else validate_resolved_url(current_url)
                    if not ok:
                        logger.warning("QQ outbound media URL redirect validation failed url={} err={}", current_url, err)
                        if resp is not None:
                            await resp.release()
                        return None, None
    ...
    async def _download_to_media_dir_chunked(
        ...
        try:
            ...
            try:
                for _ in range(5):
                    ok, err = validate_url_target(current_url) if current_url == url else validate_resolved_url(current_url)
                    if not ok:
                        logger.warning("QQ download URL redirect validation failed url={} err={}", current_url, err)
                        if resp is not None:
                            await resp.release()
                        return None

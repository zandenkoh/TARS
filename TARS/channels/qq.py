...
class QQChannel(BaseChannel):
    ...
    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        try:
            redirect_count = 0
            url_to_fetch = media_ref
            while redirect_count < 5:
                async with self._http.get(url_to_fetch, allow_redirects=False) as resp:
                    if resp.status in (301, 302, 303, 307, 308):
                        new_loc = resp.headers.get("Location")
                        if not new_loc:
                            return None, None
                        from urllib.parse import urljoin
                        url_to_fetch = urljoin(url_to_fetch, new_loc)
                        ok, err = validate_resolved_url(url_to_fetch)
                        if not ok:
                            logger.warning(f"QQ media redirect validation failed: {err}")
                            return None, None
                        redirect_count += 1
                        continue
                    ...
                    if not data:
                        return None, None
                    filename = os.path.basename(urlparse(url_to_fetch).path) or "file.bin"
            ...
            while redirect_count < 5:
                async with self._http.get(
                    ...
                    if resp.status in (301, 302, 303, 307, 308):
                        ...
                        ok, err = validate_resolved_url(url_to_fetch)
                        if not ok:
                            logger.warning(f"QQ download redirect validation failed: {err}")
                            return None
                        redirect_count += 1
                        continue

                    if resp.status != 200:
                        logger.warning("QQ download failed: status={} url={}", resp.status, url_to_fetch)
                        return None

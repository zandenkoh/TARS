...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:18790", "http://127.0.0.1:18790"],
    ...
    if tasks_dir.exists():
        for p in sorted(tasks_dir.glob("daily_*.json"), reverse=True)[:5]:
            try:
                ...
                data = await asyncio.to_thread(_load_task_file, p)
                tasks.append(
...
async def create_task(tars: TARS, content: str = Form(...), date: Optional[str] = Form(None)):
    ...
    try:
        ...
        tasks_dir = tars.workspace / "tasks"
        await asyncio.to_thread(tasks_dir.mkdir, exist_ok=True)
        ...
        if file_path.exists():
            ...
            tasks_data = await asyncio.to_thread(_load_tasks)

...
def estimate_prompt_tokens(
    ...
    try:
        ...
        for msg in messages:
            ...
            if isinstance(name := msg.get("name"), str) and name:
                append(name)

            if isinstance(tci := msg.get("tool_call_id"), str) and tci:
                append(tci)
...
def estimate_message_tokens(message: dict[str, Any]) -> int:
    ...
    if isinstance(name := message.get("name"), str) and name:
        parts.append(name)

    if isinstance(tci := message.get("tool_call_id"), str) and tci:
        parts.append(tci)

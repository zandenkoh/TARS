...
def _get_tiktoken_encoding():
    ...
    if _TIKTOKEN_ENC is None:
        # Bolt optimization: Lazy import heavy dependencies (~300ms overhead saved)
        import tiktoken
        ...
        if not tools:
            try:
                ...
                for m in messages:
                    ...
                    # Bolt optimization: explicit negative type check breaks ~10x faster on fail
...
def estimate_message_tokens(message: dict[str, Any]) -> int:
    ...
    try:
        ...
        # Bolt optimization: `type() is str` avoids `isinstance` overhead for exact matches

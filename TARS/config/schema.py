...
class Config(BaseSettings):
    ...
    def get_failover_providers(self) -> list[tuple[ProviderConfig, str, str, int]]:
        """
        ...
        if not self.agents.priority:
            sorted_prio = [(primary_name, 0)] if primary_name else []
        else:
            sorted_prio = sorted(self.agents.priority.items(), key=lambda x: x[1])

        failover = []
        for name, _ in sorted_prio:
            # Normalize config key to snake_case attribute name
            snaked_name = to_snake(name.replace("-", "_"))
            p = getattr(self.providers, snaked_name, None)
            if not p:
                continue

            # Consolidate keys: list([api_key]) + api_keys
            keys = []
            if p.api_key:

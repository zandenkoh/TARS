...
def estimate_list_comp_walrus(messages):
    try:
...
for _ in range(n): estimate_list_comp_walrus(messages_success)
t3 = time.perf_counter()

print("--- Success ---")
print(f"Old (for loop): {t1-t0:.4f}s")
print(f"List comp walrus: {t3-t2:.4f}s")
...
t2 = time.perf_counter()
for _ in range(n): estimate_list_comp_walrus(messages_fail)
t3 = time.perf_counter()

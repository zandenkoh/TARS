...
def estimate_mixed(messages):
    try:
        fast_parts = []
        append_fast = fast_parts.append
        # Combine localized loop and type check
        for m in messages:
...
for _ in range(n): estimate_mixed(messages_success)
t3 = time.perf_counter()

print("--- Success ---")
print(f"Old: {t1-t0:.4f}s")
print(f"Mixed: {t3-t2:.4f}s")
...
t2 = time.perf_counter()
for _ in range(n): estimate_mixed(messages_fail)
t3 = time.perf_counter()

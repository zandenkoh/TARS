...
## 2024-05-21 - [Accessibility: Custom Dropdown Components]
**Learning:** Custom dropdown menus built with divs and buttons lack inherent semantics. They require explicit `role="listbox"` on the container, `role="option"` on the items, and `aria-haspopup`, `aria-expanded`, and `aria-selected` attributes to communicate their state to screen readers.
**Action:** When creating or modifying custom select components, always add `role="listbox" / role="option"` and dynamically update `aria-expanded` and `aria-selected` states via JavaScript. Ensure they are keyboard focusable with visible focus rings.

...
## 2024-05-20 - [Accessibility: Missing Titles on Icon-Only Buttons]
**Learning:** Found multiple icon-only buttons (like settings, sidebar toggles, modal closes) that had `aria-label` attributes for screen readers but lacked `title` attributes for sighted users using a mouse. This missing visual tooltip makes icon-only buttons harder to understand for sighted users who don't recognize the icon.
**Action:** When adding icon-only buttons, always provide both an `aria-label` for screen reader accessibility and a `title` attribute for visual tooltips.

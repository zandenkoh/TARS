...
## 2026-04-10 - [UX: Inline Feedback vs Native Alerts]
**Learning:** Native browser `alert()` dialogs are disruptive to the user workflow and break the immersion of the custom UI. Replacing them with inline visual feedback on the action button itself provides a much smoother experience.
**Action:** When implementing save or submit actions, use the button state (e.g., "SAVING...", "SAVED!", color changes) to provide feedback instead of using `alert()`. Ensure server errors are still logged to the console for debugging.

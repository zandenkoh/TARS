...
import TARS.channels.matrix as matrix_module
from TARS.bus.events import OutboundMessage
from TARS.bus.queue import MessageBus
from TARS.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from TARS.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("TARS.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "TARS.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("TARS.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "TARS.channels.matrix.asyncio.create_task", _fake_create_task
    )

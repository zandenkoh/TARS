...
from TARS.agent.tools.web import WebSearchTool, _format_results
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="TARS", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("duckduckgo_search.DDGS", MockDDGS, raising=False)
...
def test_format_results_empty():
    query = "test query"
    res = _format_results(query, [], 5)
    assert res == f"No results for: {query}"
...
def test_format_results_missing_fields():
    items = [
        {"title": "Only Title"},
    ...
    assert "2. " in res
    assert "https://only-url.com" in res
    assert "3. " in res
    assert "Only content" in res
    assert "4. " in res


def test_format_results_limit_n():
    items = [

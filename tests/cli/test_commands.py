...
from TARS.cli.commands import _make_provider, app
from TARS.config.schema import Config
from TARS.providers.openai_codex_provider import _strip_model_prefix
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "TARS is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from TARS.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "TARS.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("TARS.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'TARS agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"TARS gateway --config {resolved_config}" in compact_output
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("TARS.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("TARS.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("TARS.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("TARS.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("TARS.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("TARS.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("TARS.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("TARS.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("TARS.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("TARS.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("TARS.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("TARS.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "TARS.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
    )
    monkeypatch.setattr(
        "TARS.cli.commands._make_provider",
...
def test_gateway_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("TARS.cron.service.CronService", _StopCron)
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from TARS.cli.commands import _migrate_cron_store
    ...
    with patch("TARS.config.paths.get_cron_dir", return_value=legacy_dir):

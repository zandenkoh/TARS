"""Tests for TARS.security.network — SSRF protection and internal URL detection."""
...
from TARS.security.network import contains_internal_url, validate_url_target
...
def test_allows_public_ip():
    with patch(
        "TARS.security.network.socket.getaddrinfo", _fake_resolve("example.com", ["93.184.216.34"])
    ):
...
def test_allows_normal_https():
    with patch(
        "TARS.security.network.socket.getaddrinfo", _fake_resolve("github.com", ["140.82.121.3"])
    ):
        ok, err = validate_url_target("https://github.com/HKUDS/TARS")
        assert ok
...
def test_detects_curl_metadata():
    with patch(
        "TARS.security.network.socket.getaddrinfo",
...
def test_detects_wget_localhost():
    with patch(
        "TARS.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])
    ):
...
def test_blocks_ipv4_mapped_ipv6():
    """Verify that IPv4-mapped IPv6 addresses targeting internal IPs are blocked."""
    ok, err = validate_url_target("http://[::ffff:127.0.0.1]/test")
    assert not ok
    assert "Blocked" in err

    ok, err = validate_url_target("http://[::ffff:169.254.169.254]/meta")
    assert not ok
    assert "Blocked" in err

    ok, err = validate_url_target("http://[::ffff:10.0.0.1]/admin")
    assert not ok
    assert "Blocked" in err

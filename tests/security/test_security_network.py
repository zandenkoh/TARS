...
def test_allows_normal_https():
    with patch("TARS.security.network.socket.getaddrinfo", _fake_resolve("github.com", ["140.82.121.3"])):
...
def test_detects_curl_metadata():
    with patch("TARS.security.network.socket.getaddrinfo", _fake_resolve("169.254.169.254", ["169.254.169.254"])):
        assert contains_internal_url('curl -s http://169.254.169.254/computeMetadata/v1/')


def test_detects_wget_localhost():
    with patch("TARS.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])):

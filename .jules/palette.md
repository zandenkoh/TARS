...
## 2024-04-20 - Contextual action accessibility
**Learning:** Contextual actions hidden by default on hover (`opacity-0 group-hover:opacity-100`) are inaccessible to keyboard users because they cannot be reached without hovering.
**Action:** Ensure accessibility by adding `focus-within:opacity-100` to their parent containers and explicit focus states (e.g., `focus-visible:opacity-100 focus-visible:ring-2`) to the interactive elements themselves. Also ensure ARIA labels for items in a list reflect the specific item rather than a generic name.

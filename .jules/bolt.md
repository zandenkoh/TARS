...
## 2026-04-01 - Avoid Multi-Key Iteration Loops in Hot Paths
**Learning:** In hot loops like `estimate_message_tokens` and `estimate_prompt_tokens`, multikey iteration loops (`for key in ("name", "tool_call_id"):`) introduce measurable tuple creation and iterator setup overhead on every iteration.
**Action:** Replace `for key in ("A", "B"): value = msg.get(key)` loops with explicit sequential conditionals using the walrus operator (`if isinstance(value := msg.get("A"), str) and value:`) to eliminate loop overhead entirely and increase throughput.

...
## 2026-04-15 - Fast Path Type Checking Optimization
**Learning:** In highly-frequent token estimation fast paths, using exact type checking (`type(obj) is str`) rather than `isinstance(obj, str)` bypasses Python's inheritance-checking overhead and executes measurably faster.
**Action:** Use exact type checks (`type(x) is type`) when inheritance support is not required and performance is critical in hot paths.

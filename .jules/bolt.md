...
## 2026-04-01 - Consolidate Guard Regexes
**Learning:** Using multiple regexes and successive `findall` calls for security guards on the same string in a hot path causes significant Python interpreter and execution overhead.
**Action:** Combine logically OR-able regex patterns into a single compiled regex pattern to eliminate redundant evaluation passes and concatenation.

...
## 2026-04-01 - Optimize TARS/utils/helpers.py hot loops

**Learning:** In Python hot loops (like token estimation and string chunking), caching instance methods locally (e.g., `append = parts.append`, `dumps = json.dumps`) prior to the loop significantly reduces attribute lookup overhead. Furthermore, replacing multi-key iteration loops with direct, sequential conditionals using the walrus operator eliminates tuple creation and iteration overhead, yielding O(1) early rejections.
**Action:** When working on tokenizing or processing multiple messages, cache methods locally and utilize walrus operators within the loop body.

...
## 2026-04-02 - Exact Type Checks over isinstance in Hot Paths
**Learning:** Using `isinstance(obj, str)` inside performance-critical loops (like fast paths for token estimation) incurs unnecessary overhead when checking for exact types, because it has to traverse the inheritance tree.
**Action:** When exact type checking is sufficient and inheritance support is not required (e.g. `str`), use `type(obj) is str` instead to bypass the inheritance checker and execute faster.

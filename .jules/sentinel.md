...
## 2026-04-01 - Missing CSRF Protection in WebUI State-Changing Endpoints

**Vulnerability:** State-changing endpoints (e.g., `/api/config`, `/api/channels/toggle`) in the TARS local WebUI accepted HTTP POST/DELETE requests (from HTMX forms) without checking the `Origin` or `Referer` headers. A malicious website visited by the user could automatically submit a form to `http://localhost:18790/api/config`, executing a Cross-Site Request Forgery (CSRF) attack to maliciously modify the user's settings or execute other unintended actions.
**Learning:** Even when CORS is strictly configured to only allow localhost origins to *read* responses, it does not prevent browsers from *sending* simple POST requests (e.g., form submissions) cross-origin. In a local dashboard without cookies, CSRF protection is still mandatory.
**Prevention:** Implement strict `Origin` or `Referer` validation in the middleware (e.g., `SecurityHeadersMiddleware`) for all state-changing HTTP methods (`POST`, `PUT`, `DELETE`, `PATCH`). Ensure the host matches the expected local application origins (`localhost:18790`, `127.0.0.1:18790`).
